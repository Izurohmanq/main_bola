...
var command = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err = db.AutoMigrate(&models.Order{}, &models.OrderHistory{}, &models.OrderField{})
...
func serverKafkaConsumer(service services.IServiceRegistry) {
	...
	kafkaConsumerConfig.Consumer.MaxWaitTime = time.Duration(config.Config.Kafka.MaxWaitTimeInMs) * time.Millisecond             // waktu tunggu sebelum diteruskan ke consumer
	kafkaConsumerConfig.Consumer.MaxProcessingTime = time.Duration(config.Config.Kafka.MaxProcessingTimeInMs) * time.Millisecond // waktu maksimal pemroresesan di kafka
	kafkaConsumerConfig.Consumer.Retry.Backoff = time.Duration(config.Config.Kafka.BackOffTimeInMs) * time.Millisecond           // waktu tunggu jika ada kesalahan sebelum dijalankan kembali
	...
	if err != nil {
		logrus.Errorf("failed to crete consumer group: %v", err)
		return
	}
	...
	signals := make(chan os.Signal, 1) // menerima sinyal dari OS
	signal.Notify(signals, os.Interrupt, syscall.SIGTERM)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	// digunakan untuk kafka intinya
